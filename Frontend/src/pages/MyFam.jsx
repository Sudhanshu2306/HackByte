...
function MyFam() {
  ...
  const handleSearchInputChange = (e) => {
    ...
    setSearchTerm(query);
    if (query) handleAutocomplete(query);
    else setSuggestions([]);
  };
  ...
  const handleLocationUpdate = (place) => {
    ...
    if (map) map.flyTo({ center: [place.lon, place.lat], zoom: 14 });
  };
  ...
  return (
    <div className="flex flex-col h-screen p-4 bg-gray-white">
      ...
      <div className="flex flex-1 rounded-2xl shadow-lg overflow-hidden gap-2 border-2">
        {/* Left Sidebar */}
        {isMenuOpen && (
          <div className="w-1/4 p-4 flex flex-col space-y-4 overflow-y-auto border-r-2 rounded-xl">
            {userFamily.length > 0 ? (
